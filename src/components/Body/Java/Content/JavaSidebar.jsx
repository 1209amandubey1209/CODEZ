import React,{useState} from "react";
import "../Java.css";
import { JavaContentList } from "../../../../Data/Backend/backendContentList";
import { NavLink } from "react-router-dom";
import MenuIcon from '@mui/icons-material/Menu';

function contentList(JavaContentList) {
  return (
    <>
     <h3>{JavaContentList.course}</h3>
     <NavLink
      style={{ color: "black", textDecoration: "none" }}
      to={JavaContentList.path}
    >
      <div className="side-item">{JavaContentList.title}</div>
    </NavLink>
    </>
  );
}

function JavaSidebar() {
  const [status,setStatus]=useState(false)
  return (
    <>
      <div className="position-fixed d-lg-block  d-none overflow-auto " style={{ height: "91%" }}>
        {JavaContentList.map(contentList)}
      </div>
      <div className='d-bock d-lg-none position-fixed overflow-auto '  onClick={()=>setStatus(!status)}><MenuIcon/></div>
      {status?
      <div className="col d-block d-lg-none position-fixed overflow-auto bg-white" style={{ height: "91%" }} onClick={()=>setStatus(!status)}>
      {JavaContentList.map(contentList)}
    </div>
      :null
       }
    </>
  );
}

export default JavaSidebar;
